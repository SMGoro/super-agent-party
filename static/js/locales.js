      ...
      'bodyPlaceholder': `请输入请求体，使用JSON Schema格式。例如：
{
    ...
    "required": ["name", "age"]
        ...
        'browserNotice': '1. 在终端中运行`npm install -g mcp-chrome-bridge`全局安装mcp-chrome-bridge',
        'browserNotice1': '2. 点击上方的github链接，下载chrome-mcp-server.zip文件',
        'browserNotice2': '3. 访问`chrome://extensions/`，打开浏览器的开发者模式，加载未打包的扩展程序，上传chrome-mcp-server.zip文件，启用插件',
        'browserNotice3': '4. 在新扩展页面点击连接，将页面中的URL复制到基础URL中，默认为`http://127.0.0.1:12306/mcp`可不替换。',
      ...
      'bodyPlaceholder': `Enter the body in JSON Schema format. For example:
{
    ...
    "required": ["name", "age"]
        ...
        'browserNotice': '1. Run `npm install -g mcp-chrome-bridge` in the terminal to globally install mcp-chrome-bridge',
        'browserNotice1': '2. Click the GitHub link above to download the chrome-mcp-server.zip file',
        'browserNotice2': '3. Visit `chrome://extensions/`, enable Developer Mode in your browser, select "Load unpacked" to upload the chrome-mcp-server.zip file, and enable the extension',
        'browserNotice3': '4. Click "Connect" on the new extension page, then copy the URL from the page into the Base URL field. The default is `http://127.0.0.1:12306/mcp`, which does not need to be replaced if unchanged.',
