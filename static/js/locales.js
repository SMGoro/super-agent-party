      ...
      'bodyPlaceholder': `请输入请求体，使用JSON Schema格式。例如：
{
    ...
    "required": ["name", "age"]
        ...
        'browserNotice4': '1. 选择`内置浏览器`则会控制本软件中的内置浏览器，【浏览器模式】下`内置浏览器`选项不可用',
        'browserNotice5': '2. 你的电脑需要安装node环境，版本必须是v20.19或者更新的LTS版本，可以从 `https://nodejs.org/en/download` 下载安装',
      ...
      'bodyPlaceholder': `Enter the body in JSON Schema format. For example:
{
    ...
    "required": ["name", "age"]
        ...
        'browserNotice4': '1. Selecting `Internal Browser` will control the browser within this software. The `Internal Browser` option is unavailable in [Browser Mode].',
        'browserNotice5': '2. Your computer needs to have the Node.js environment installed. The version must be v20.19 or a newer LTS version. You can download and install it from `https://nodejs.org/en/download`.',
